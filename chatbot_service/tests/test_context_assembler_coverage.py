...
class TestAddEmergencyContext:
    @pytest.mark.asyncio
    async def test_without_lat_lon_does_nothing(self):
        retriever = FakeRetriever()
        ...
        await assembler._add_emergency_context(ctx)
        assert len(ctx.tools) == 0

    @pytest.mark.asyncio
    async def test_with_lat_lon_adds_sos_and_weather(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(),
        ...
        assert "sos" in tool_names
        assert "weather" in tool_names

    @pytest.mark.asyncio
    async def test_with_what3words(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
        ...
        ctx = make_context(lat=13.0, lon=80.0)
        await assembler._add_emergency_context(ctx)
        sos_tools = [t for t in ctx.tools if t.name == "sos"]
        assert len(sos_tools) == 1
        assert "What3Words" in sos_tools[0].summary

    @pytest.mark.asyncio
    async def test_without_what3words(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(include_w3w=False),
            challan_tool=FakeChallanTool(),
            legal_search_tool=FakeLegalSearchTool(retriever),
            first_aid_tool=FakeFirstAidTool(),
            road_infra_tool=FakeRoadInfrastructureTool(),
        ...
        sos_tools = [t for t in ctx.tools if t.name == "sos"]
        assert len(sos_tools) == 1
        assert "What3Words" not in sos_tools[0].summary

    @pytest.mark.asyncio
    async def test_empty_sos_payload_skipped(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(return_empty=True),
            challan_tool=FakeChallanTool(),
            legal_search_tool=FakeLegalSearchTool(retriever),
            first_aid_tool=FakeFirstAidTool(),
            road_infra_tool=FakeRoadInfrastructureTool(),
    ...
    @pytest.mark.asyncio
    async def test_empty_weather_skipped(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(),
...
class TestAddRoadContext:
    @pytest.mark.asyncio
    async def test_without_lat_lon_still_offers_report_guidance(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(),
        ...
        ctx = make_context(lat=None, lon=None, message="report this road hazard")
        await assembler._add_road_context(ctx)
        assert len(ctx.tools) == 1
        assert ctx.tools[0].name == "submit_report"

    @pytest.mark.asyncio
    async def test_with_lat_lon_adds_infra_and_issues(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(),
        ...
        ctx = make_context(lat=13.0, lon=80.0, message="report a pothole")
        await assembler._add_road_context(ctx)
        tool_names = [t.name for t in ctx.tools]
        assert "road_infrastructure" in tool_names
        assert "road_issues" in tool_names
        assert "submit_report" in tool_names

    @pytest.mark.asyncio
    async def test_empty_infra_skipped(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(),
        ...
        ctx = make_context(lat=13.0, lon=80.0, message="report")
        await assembler._add_road_context(ctx)
        infra_tools = [t for t in ctx.tools if t.name == "road_infrastructure"]
        assert len(infra_tools) == 0

    @pytest.mark.asyncio
    async def test_empty_issues_skipped(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(),
            ...
            first_aid_tool=FakeFirstAidTool(),
            road_infra_tool=FakeRoadInfrastructureTool(),
            road_issues_tool=FakeRoadIssuesTool(issues=[]),
            submit_report_tool=FakeSubmitReportTool(),
            weather_tool=FakeWeatherTool(),
            drug_info_tool=FakeDrugInfoTool(),
        )
        ctx = make_context(lat=13.0, lon=80.0, message="report")
        await assembler._add_road_context(ctx)
        issues_tools = [t for t in ctx.tools if t.name == "road_issues"]
        assert len(issues_tools) == 0

    @pytest.mark.asyncio
    async def test_no_report_keyword_no_submit_report(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(),
        ...
        ctx = make_context(lat=13.0, lon=80.0, message="check road condition")
        await assembler._add_road_context(ctx)
        report_tools = [t for t in ctx.tools if t.name == "submit_report"]
...
class TestAddRouteContext:
    @pytest.mark.asyncio
    async def test_without_location_adds_base_tool_only(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(),
        ...
        ctx = make_context(lat=None, lon=None)
        await assembler._add_route_context(ctx)
        assert len(ctx.tools) == 1
        assert ctx.tools[0].name == "safe_route"
        assert ctx.tools[0].payload.get("requires_origin_destination") is True

    @pytest.mark.asyncio
    async def test_with_location_adds_issues_and_weather(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(),
        ...
        await assembler._add_route_context(ctx)
        tool_names = [t.name for t in ctx.tools]
        assert "safe_route" in tool_names
        assert "route_risk" in tool_names
        assert "route_weather" in tool_names

    @pytest.mark.asyncio
    async def test_with_location_no_issues_no_weather(self):
        retriever = FakeRetriever()
        assembler = ContextAssembler(
            retriever=retriever,
            sos_tool=FakeSosTool(),
            ...
            first_aid_tool=FakeFirstAidTool(),
            road_infra_tool=FakeRoadInfrastructureTool(),
            road_issues_tool=FakeRoadIssuesTool(return_empty=True),
            submit_report_tool=FakeSubmitReportTool(),
            weather_tool=FakeWeatherTool(return_empty=True),
            drug_info_tool=FakeDrugInfoTool(),
        )
...
class TestAssembleIntentPaths:
    ...
    async def test_submit_report_context_offers_guidance(self, assembler):
        ...
        assert len(report_tools) == 0  # assemble() uses inline code, not _add_road_context
