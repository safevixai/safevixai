...
export default function ProfileError({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => { logClientError('Profile page crashed:', error) }, [error]);
  return (
      ...
      <section className="w-full max-w-xl border border-red-500/25 bg-red-950/20 rounded-lg p-6 shadow-2xl">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-3">
            <p className="text-xs uppercase tracking-[0.24em] text-red-200/70">Profile</p>
            <h1 className="text-2xl font-black tracking-tight">Profile unavailable</h1>
            <p className="text-sm leading-6 text-text-3">Your profile could not be loaded. Please try again.</p>
