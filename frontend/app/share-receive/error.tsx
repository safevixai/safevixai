...
import { Share2, Home, Phone, RefreshCw } from 'lucide-react';
import { logClientError } from '@/lib/client-logger';

export default function ShareReceiveError({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => { logClientError('Share-Receive page crashed:', error) }, [error]);
  return (
    <main className="min-h-screen bg-bg text-text-1 flex items-center justify-center px-6">
      <section className="w-full max-w-xl border border-red-500/25 bg-red-950/20 rounded-lg p-6 shadow-2xl">
        ...
        <div className="flex items-start gap-4">
          <div className="mt-1 rounded-full bg-red-500/15 p-3 text-red-300"><Share2 size={24} /></div>
          <div className="space-y-3">
            <p className="text-xs uppercase tracking-[0.24em] text-red-200/70">Share & Receive</p>
            <h1 className="text-2xl font-black tracking-tight">Share service unavailable</h1>
            <p className="text-sm leading-6 text-text-3">The sharing feature encountered an error.</p>
